...
The slope coefficient (0.693) is not statistically significant.  
...
mod1
ggplot(kidiq, aes(x = kid_score, y = mom_age)) +
  geom_point() +
  geom_smooth(method = "lm", se = F) +
  labs(x = "kid test score", y = "mom age")
...
Assumptions:  From the residual vs. fitted plot and the scale-location plot, we need to investigate the condition of constant variance. The center seems differ slightly from both ends. The Q-Q plot is still linear, so the residuals are normal. The residual vs. leverage plot need not to concern us, since nothing seems to go beyond and dashed line.
...
mod3 <-
  stan_glm(
   kid_score ~ mom_hs + mom_age + mom_hs:mom_age,
    data = kidiq,
    refresh = 0
  )
mod3
...
When both are in the model, it seems, without high school education, a kid's score declines as mother's age increases.  
...
(Test sample is slightly larger than 200 because the total is 433. Again, use mom_iq instead of education.)  
\
```{r}
...
pred <- predict(mod5)
res <- resid(mod5)
shape_num <- ifelse(x2 == 0, 1, 19)
plot(pred, res, pch = shape_num)
```
...
Explain whether each scenario is a classification or regression problem, and indicate whether we are most interested in inference or pre- diction. Finally, provide n and p.  
