...
The relationship appears to be positive and reasonably linear.  
...
Since we are only looking at observational data, it is NOT reasonable to make causal conclusions. There could be other factors at issue that are not accounted for in the observation. 
...
The range of the data of white households poverty rate used for creating the model is between 5% and 17%. 85% is too much beyond the data range, therefore, it is not a good idea to predict Trump support at 85% white poverty based on the current model.  
...
ggplot(data = trump, aes(y = trump_support, x = non_white)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Percentage of non_white households in poverty", 
       y = "Percentage of voters supporting Trump", 
       title = "Non_white poverty and Trump support in the US") 
```
...
The correlation coefficient (for `non_white` and `trump_support`) is expected to be negative.  
\
```{r}
trump %>%
...
get_regression_table(non_white_mod)
...
ggplot(data = trump, aes(y = trump_support, x = non_white))+ 
  ...
  labs(x = "Percentage of non_white households in poverty", 
       y = "Percentage of voters supporting Trump", 
       title = "Non_white poverty and Trump support in the US") + 
  geom_abline(slope = -0.269, intercept = 58.328, col = "black", lwd = 0.5) +
  geom_smooth(method = "lm", se = FALSE, col = "red", lwd = 2 ) 

get_regression_points(non_white_mod)
```

**Answer:**  

* When the non_white poverty level is 0%, the predicted average Trump support is 58.328%.  
* For every percent increase in non_white poverty, there is an **associated decrease** in Trump support of 0.269 percentage point. 
...
The correlation coefficients are close (different direction), -0.4250604 for non_white vs. 0.4864114 for white, showing only a slightly stronger relation between white poverty and Trump support. The scatter plots seem to show a slightly larger variance in the non_white model. This can also find evidence in the r2 values for the models: 0.236596 for white poverty vs. 0.1806764 for non_white, suggesting the white poverty models explains a larger proportion of the variance in the data. Overall, the white poverty models seems to be a better explanatory variable of Trump support, although only by a small margin.
...
\
```{r}
get_regression_points(non_white_mod) %>% filter(non_white == 61)

33 - 41.912
```
Overestimate, by 8.912.

**C.** In other words, what is the residual for this prediction? 

```{r}
get_regression_points(non_white_mod) %>% filter(non_white == 61) %>% .[5]
```
