...
**b = 15 / 10 = 1.5, a = 100 - 1.5 * 35 = 47.5**

(b) What is the range of possible values of this rescaled score y?  

**new_range = scale * old_range = 1.5 * old_range = [0, 150]**

(c) Plot the line showing y vs. x. 
\
```{r}
scores <- tibble(x = 0:150, prob = dnorm(x, 100, 15))

ggplot(data = scores, mapping = aes(x = x, y = prob)) +
  geom_col(fill = 'purple') +
  ggtitle("Probability Mass Function for Normal Distribution(mean = 100, std = 15)") +
  xlab("score") + ylab("Pr(Score)")
```
