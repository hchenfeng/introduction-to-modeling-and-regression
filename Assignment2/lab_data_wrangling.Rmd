...
For this problem set, you are being given a R Markdown file to fill your answers into!  You should have opened this file here on the Rstudio server. Do save as to save the file into your STA631 directory under Home.

Fill in the answers below the corresponding Question headers. Be sure to include text and code where necessary. 
...
dallas_sub <- txhousing %>%
  filter(city == "Dallas", year == (2012:2013))
```
...
```{r}
txhousing %>% 
  # select rows whose year value is either 2012 or 2013, and city value is "Dallas"
  filter(year == 2012 | year == 2013, city == "Dallas") %>% 
  # add a new column named "prct_sold", whose values is calculated through "sales/listings *100",
  # which shows the percentage of houses sold
  mutate(prct_sold = sales/listings *100) %>%
  # group rows by month
  group_by(month) %>%
  # summarize the average percentage of houses sold each month
  summarize(mean_prct_sold = mean(prct_sold)) %>% 
  # show rows by average percentage of houses sold in descending order
...
txhousing %>% 
  filter(year == 2012, month == 1) %>% 
  slice(which.min(listings))
```
**In January of 2015, the city that had the fewest houses listed for sale was San Marcos.**
...
**In 2012, August was the month when the most houses were sold in Texas.**
...
```{r results='show'}
txhousing %>%
  filter((city == "Austin" | city == "Dallas"), year == (2000:2001)) %>%
  group_by(city) %>%
