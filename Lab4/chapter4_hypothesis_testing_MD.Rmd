...
author: "Chenfeng Hao"
date: "`r format(Sys.time(), tz = 'ET', '%b %d, %Y')`"
output: pdf_document
mainfont: Arial
---
...
```{r}
mean_gpa_by_gender <-
  sat_gpa %>% group_by(sex) %>% summarise(mean_gpa = mean(gpa_fy))
mean_gpa_by_gender[1, "mean_gpa"] - mean_gpa_by_gender[2, "mean_gpa"]
```
...
2. This difference does not seem statistically significant.
...
```{r}
ggplot(data = sat_gpa, aes(x = sex, y = gpa_fy)) +
  geom_boxplot(fill = "blue") +
  ggtitle("GPAs of the Female and Male groups") +
...
1. Same as the original, 1000. 

2. We shuffled 5000 times, and there are 5,000,000 rows in the `gpas_in_null_world` data frame.
...
1. 5000, because we shuffled 5000 times.
...
The mean GPA scores for females in our sample ($\bar{x}$ = 2.54) was greater than that of males ($\bar{x}$ = 2.40). This difference was statistically significant at  $\alpha = 0.05$, (p = 0.02).  Given this I would reject the Null hypothesis and conclude that females have higher GPAs than males at the population level. 
...
1. There is no difference between the mean total SAT scores of the low and high GPA high-school students.
...
visualize(sat_diff_under_null) +
  ...
  labs(x = "Difference in SAT total for high and low GPAs", y = "Count",
       title = "Null distribution of SAT total for high and low GPAs",
       subtitle = "Actual difference observed in the data is marked in red"
       ) +
...
1. The mean total SAT scores for students with high GPAs in our sample ($\bar{x}$ = 108.68) was greater than that of students with low GPAs ($\bar{x}$ = 98.23). This difference was statistically significant at  $\alpha = 0.05$, (p < 0.001).  Given this I would reject the Null hypothesis and conclude that students with high GPAs have higher mean total SAT scores than students with low GPAs.
